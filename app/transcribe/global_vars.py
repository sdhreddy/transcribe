...
class TranscriptionGlobals(Singleton.Singleton):
    ...
    def initiate_audio_devices(self, config: dict):
        ...
        if not config['General']['disable_mic']:
            ...
            if not config['General']['disable_mic']:
                try:
                    # Check if we should use simple continuous recording
                    if config['General'].get('use_simple_continuous_recording', False) and ar.SimpleMicRecorder:
                        print('[INFO] Using SimpleMicRecorder for continuous recording')
                        recording_interval = config['General'].get('simple_recording_interval', 3.0)
                        self.user_audio_recorder = ar.SimpleMicRecorder(
                            source_name='You',
                            recording_interval=recording_interval
                        )
                        print(f'[INFO] Simple recording with {recording_interval}s intervals')
                    else:
