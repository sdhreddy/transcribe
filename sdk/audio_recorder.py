...
# Mock recorder for when no audio devices available
try:
    from .mock_mic_recorder import MockMicRecorder
except ImportError:
    MockMicRecorder = None

# Simple continuous recorder option
try:
...
ENERGY_THRESHOLD = 600  # More sensitive to capture quieter colleague voices
...
class BaseRecorder:
    ...
    def __init__(self, source, source_name, audio_file_name: str = None):
        ...
        # Use original energy threshold
        self.recorder.energy_threshold = ENERGY_THRESHOLD  # Back to 1000
        self.recorder.dynamic_energy_threshold = DYNAMIC_ENERGY_THRESHOLD
        # Set pause threshold to control when speech is considered complete
        self.recorder.pause_threshold = 0.6  # Faster speech detection (was 0.8)
        self.recorder.non_speaking_duration = 0.5  # Minimum non-speaking audio to keep
    ...
    def __init__(self, source_name='You', audio_file_name: str = None):
        ...
        try:
            ...
            if inputs:
                ...
                # Try to use mock recorder if available
                if MockMicRecorder:
                    logger.warning("No audio devices found - using MockMicRecorder for testing")
                    mock_instance = MockMicRecorder(audio_file_name)
                    # Copy attributes from mock to self
                    self.__dict__.update(mock_instance.__dict__)
                    self.__class__ = MockMicRecorder
                    return
                else:
